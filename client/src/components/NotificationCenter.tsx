...
export function NotificationCenter({ onOpenWorkboard, notices, onMarkRead, onMarkAllRead, defaultOpen = false }: { onOpenWorkboard: () => void; notices: Notice[]; onMarkRead: (id: number) => void; onMarkAllRead: () => void; defaultOpen?: boolean }) {
  ...
  return (
    <div className="notification-center">
      <button className="notification-trigger" type="button" onClick={() => setOpen((current) => !current)} aria-label="Open notifications" aria-expanded={open}>
        <Bell size={16} />
      ...
      {open && <div className="notification-panel" role="status" aria-label="Notifications">
        ...
        <div className="notification-list">
          {notices.map((notice) => <button key={notice.id} type="button" className={`notification-item ${notice.unread ? "unread" : ""}`} onClick={() => openNotice(notice.id)}>
            <span className="notification-symbol">{notice.type === "message" ? <MessageCircle size={15} /> : <Send size={14} />}</span>
            <span><b>{notice.title}</b><small>{notice.detail}</small><time>{notice.time}</time></span>
            {notice.unread && <i />}
          </button>)}
        </div>
        <button type="button" className="notification-clear" onClick={onMarkAllRead}><Check size={14} /> Mark all as read</button>
      </div>}
    </div>
  );
}
