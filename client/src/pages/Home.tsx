  ...
  return (
    <div className="relay-app" id="top">
      <header className="relay-header">
        ...
        <div className="header-actions"><ThemeToggle /><NotificationCenter onOpenWorkboard={() => scrollTo("dashboard")} notices={notifications} onMarkRead={markNotificationRead} onMarkAllRead={markAllNotificationsRead} defaultOpen={notificationPreview} /><a className="header-login" href="/dashboard/freelancer">Dashboard</a><button className="header-cta" type="button" onClick={openPosting}>Post a project <ArrowUpRight size={15} /></button></div>
      ...
      <main>
        <section className="relay-hero" aria-labelledby="hero-title"><div className="hero-copy"><motion.p className="section-kicker" {...reveal}><i /> Independent work, better connected</motion.p><motion.h1 id="hero-title" {...reveal} transition={{ ...reveal.transition, delay: .04 }}>Good work <em>finds</em> its way.</motion.h1><motion.p className="hero-description" {...reveal} transition={{ ...reveal.transition, delay: .08 }}>A focused marketplace for specialists and thoughtful teams. Discover clear briefs, make a case for your craft, and keep every opportunity in one place.</motion.p><motion.div className="hero-actions" {...reveal} transition={{ ...reveal.transition, delay: .12 }}><button className="primary-button" type="button" onClick={() => scrollTo("projects")}>Explore open briefs <ArrowDownRight size={18} /></button><button className="text-button" type="button" onClick={openPosting}>Post a client brief <Plus size={17} /></button></motion.div><motion.div className="hero-trustline" {...reveal} transition={{ ...reveal.transition, delay: .16 }}><span><b>146</b> active briefs</span><span><b>24h</b> typical response window</span><span><b>One place</b> for the full thread</span></motion.div></div><motion.div className="hero-visual" {...reveal} transition={{ ...reveal.transition, delay: .1 }}><img src={assets.hero} alt="Editorial creative studio worktable with project planning materials" /><div className="hero-brief-card"><p>OPEN BRIEF <span>⌁</span></p><strong>Mobile checkout redesign</strong><div><span>Product design</span><b>$2.4k–$3.2k</b></div></div><div className="hero-stamp">MATCH<br />THE<br />WORK</div><div className="hero-index">01 / START HERE</div></motion.div></section>

        <section className="category-section relay-shell" aria-labelledby="category-title"><motion.div className="section-title-row" {...reveal}><div><p className="section-kicker">01 / EXPLORE BY CRAFT</p><h2 id="category-title">A good brief starts with the right <em>discipline.</em></h2></div><button type="button" className="mini-link" onClick={() => scrollTo("projects")}>All open briefs <ArrowUpRight size={16} /></button></motion.div><div className="category-grid">{categories.map((item, index) => <motion.button key={item.label} className={`category-card tone-${item.tone}`} type="button" {...reveal} transition={{ ...reveal.transition, delay: index * .045 }} onClick={() => { setCategory(item.label); scrollTo("projects"); }}><span className="category-icon">{item.icon}</span><span className="category-label">{item.label}</span><span className="category-count">{item.count} briefs <ArrowUpRight size={14} /></span></motion.button>)}</div></section>
