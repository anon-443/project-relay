...
A final production audit compared the managed site (`orbitfolio-fbbkuhat.manus.space`) and GitHub Pages (`anon-443.github.io/project-rely`) at the same desktop viewport. Both expose the same navigation, hero, category cards, twelve projects, talent directory, workboard, conversation panel, and footer because they now render the shared `Home` component. The only intentional difference is account state: GitHub Pages is static and shows a Sign in link that opens the managed app, while the managed site reflects the current browser session.
